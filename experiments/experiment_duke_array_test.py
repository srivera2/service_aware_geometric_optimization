...
from sionna.rt import load_scene, Receiver, Camera, PathSolver, AntennaArray, PlanarArray, RadioMapSolver
...
from multi_tx_optimizer import (
    ...
    make_coverage_gif_callback, frames_to_gif
)
...
# Step 1 of the num_tx_ant 1->2->4 walk: exactly 2 elements, uniform/fixed
# combining only (learn_precoding stays False in _base_hparams below).
...
SIZE_CONFIGS = [
    {"name": "medium",  "size_m": 300.0, "n_bs": 2, "standoff_m": 80.0}
]

SAMPLE_COUNTS = [750]

OUTPUT_ROOT = pathlib.Path(f"../results/{SCENE_NAME}_precoding_enabled_4_element_half_wavelength")
...
def _seed_jammers_uniform(zone_params, shape=None, n_jammers=8, standoff_distance=75.0, **_):
    """Place n_jammers equally spaced along the zone boundary, offset outward by
    standoff_distance metres.

    Uses a true perpendicular buffer (Minkowski offset) rather than affine
    scaling about the centroid. Centroid scaling gives zero clearance on any
    edge that passes through the center -- e.g. the semi-circle's flat
    diameter edge -- since points on that line only slide along it instead of
...
for shape in SHAPES:
    for size_cfg in SIZE_CONFIGS:
        for n_samples in SAMPLE_COUNTS:
            ...
            jam_configs = [
                JammerConfig(name=f"jam_{k+1}", initial_power_dbm=30.0,
                             power_dbm_bounds=(10.0, 40.0),
            ...
            # ── Run 1: BS-only warm-up (100 iter) — inside coverage only ─────────
            ...
            _warmup_kwargs = {**_shared, "lambda_in": 10.0, "lambda_out": 0.0, "soft_mean_weight": 0.0}
            ...
            #
            # A coverage-map frame (same visual as compare_multi_tx_performance's
            # map panel, including this iteration's actual precoding weights)
            # is rendered every other iteration and assembled into a progress
            # GIF below, so the BS+jammer SINR field's evolution can be reviewed
            # without re-running the optimisation.
            print("  Run 2: Joint optimization (100 iter, BS positions not frozen)")
            gif_cb = make_coverage_gif_callback(
                map_config=MAP_CONFIG,
                zone_masks=zone_masks_dict,
                output_dir=str(run_dir / "gif_frames"),
                jam_configs=jam_configs,
                frame_every=2,
                noise_power=_base_hparams["noise_power"],
                gamma_db=_base_hparams["gamma_db"],
            ...
            if gif_cb.frame_paths:
                gif_path = frames_to_gif(
                    gif_cb.frame_paths, str(run_dir / "joint_progress.gif"), fps=8
                )
                print(f"  Progress GIF: {gif_path} ({len(gif_cb.frame_paths)} frames)")
            ...
            results = {
                ...
                "params": {
                    **_base_hparams,
                    ...
                    "num_iterations_warmup":    60,
                    "num_iterations_joint":     60,
