...
def prepare_triangulated_sampler(tri_verts):
    """Pre-compute Morton sort + cumulative areas for a triangulated zone.

    Called once per zone (or once when the alive-zone geometry changes).
    The returned dict is passed to ``sample_from_prepared`` on every iteration,
    avoiding the redundant sort + area computation that ``sample_triangulated_zone``
    performs from scratch each call.

    Parameters
    ----------
    tri_verts : (N, 3, 2) ndarray — triangle vertices from ``triangulate_zone``

    Returns
    -------
    dict with keys: 'A', 'B', 'C' (sorted vertex arrays), 'areas', 'total_area'
    """
    def _spread_bits(v):
        v = (v | (v << 8)) & 0x00FF00FF
        v = (v | (v << 4)) & 0x0F0F0F0F
    ...
    norm         = (centroids - min_b) / (max_b - min_b + 1e-8)
    coords       = (norm * 65535).astype(np.uint32)
    morton_codes = _spread_bits(coords[:, 0]) | (_spread_bits(coords[:, 1]) << 1)
    ...
    return {"A": A, "B": B, "C": C, "areas": areas, "total_area": float(np.sum(areas))}


def sample_from_prepared(prepared, num_samples, qrand=None, ground_z=0.0):
    """Sample uniformly from a pre-sorted triangulated zone.

    Uses the precomputed geometry from ``prepare_triangulated_sampler`` so
    Morton sorting and area computation are skipped on every call.

    Parameters
    ----------
    prepared    : dict returned by ``prepare_triangulated_sampler``
    num_samples : int
    qrand       : scipy QMC sampler or None (falls back to numpy uniform)
    ground_z    : float — z coordinate assigned to all output points

    Returns
    -------
    (num_samples, 3) float32 array of [x, y, z] positions
    """
    A, B, C    = prepared["A"], prepared["B"], prepared["C"]
    areas      = prepared["areas"]
    total_area = prepared["total_area"]
    ...
    if qrand is not None:
        raw = np.array(qrand.random(actual_total))
        if raw.shape[1] < 2:
            raw = np.hstack([raw, np.array(qrand.random(actual_total))])
        u1, u2 = raw[:, 0], raw[:, 1]
    else:
        u1 = np.random.random(actual_total)
        u2 = np.random.random(actual_total)

    sqrt_r1 = np.sqrt(u1)
    w_A = (1 - sqrt_r1)[:, None]
    w_B = (sqrt_r1 * (1 - u2))[:, None]
    w_C = (sqrt_r1 * u2)[:, None]

    pts_2d = (w_A * A[triangle_indices]
              + w_B * B[triangle_indices]
              + w_C * C[triangle_indices])
    z_col = np.full((actual_total, 1), ground_z)
    return np.hstack([pts_2d, z_col]).astype(np.float32)
